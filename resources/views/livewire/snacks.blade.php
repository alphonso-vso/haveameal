        ...
        <div class="flex flex-wrap -m-4">
            ...
            <div class="p-4 xl:w-1/4 md:w-1/2 w-full">
                <div class="h-full p-6 rounded-lg border-2 border-pink-500 flex flex-col relative overflow-hidden bg-white">
                    ...
                    @foreach(explode(',', $snack->ingredients) as $ingredient)
                    ...
                    @endif
                    @role('super-admin')
                    <button wire:click="confirmMealEdit({{ $snack->id }})" class="flex items-center mt-auto text-white bg-rose-500 border-0 py-2 px-4 w-full focus:outline-none hover:bg-rose-600 rounded transition ease-in-out duration-150">{{ __('Editar') }}
                        <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-4 h-4 ml-auto" viewBox="0 0 24 24">
                            <path d="M5 12h14M12 5l7 7-7 7"></path>
                        </svg>
                    </button>
                    @else
                    @if (strtoupper($dayName) == strtoupper($snack->day))
                    <button wire:click="confirmMealOrdering({{ $snack->id }})" class="flex items-center mt-auto text-white bg-emerald-500 border-0 py-2 px-4 w-full focus:outline-none hover:bg-emerald-600 rounded transition ease-in-out duration-150">{{ __('Ordenar') }}
                        <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-4 h-4 ml-auto" viewBox="0 0 24 24">
                            <path d="M5 12h14M12 5l7 7-7 7"></path>
                        </svg>
                    </button>
    ...
    <x-dialog-modal wire:model="confirmingMealOrder">
        ...
        <x-slot name="content">
            <div class="col-span-6 sm:col-span-4" hidden>
                ...
                <x-input id="meal_id" type="number" class="mt-1 block w-full" wire:model.defer="snack.meal_id" />
                <x-input-error for="meal_id" class="mt-2" />
            </div>
            <div class="col-span-6 sm:col-span-4">
